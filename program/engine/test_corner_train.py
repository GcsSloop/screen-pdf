...
class CornerTrainTests(unittest.TestCase):
    ...
    def test_corner_sample_dataset_reuses_cached_base_item_for_duplicate_roi_paths(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            roi_dir = root / "roi" / "train"
            roi_dir.mkdir(parents=True)
            image = np.zeros((120, 160, 3), dtype=np.uint8)
            image_path = roi_dir / "shared.png"
            cv2.imwrite(str(image_path), image)
            rows = [
                {
                    "split": "train",
                    "page_id": "page-1",
                    "roi_path": "roi/train/shared.png",
                    "corner_norm": [[0.1, 0.2], [0.8, 0.2], [0.8, 0.8], [0.1, 0.8]],
                    "coarse_quad_norm": [[0.12, 0.22], [0.78, 0.2], [0.79, 0.78], [0.11, 0.81]],
                },
                {
                    ...
                    "roi_path": "roi/train/shared.png",
                    "corner_norm": [[0.1, 0.2], [0.8, 0.2], [0.8, 0.8], [0.1, 0.8]],
                    "coarse_quad_norm": [[0.13, 0.21], [0.77, 0.2], [0.78, 0.79], [0.12, 0.8]],
                },
            ]
            (root / "train.jsonl").write_text("\n".join(json.dumps(row, ensure_ascii=False) for row in rows) + "\n", encoding="utf-8")
        ...
        self.assertIs(dataset._cache[0]["image"], dataset._cache[1]["image"])
        self.assertFalse(np.shares_memory(dataset._cache[0]["coarse"], dataset._cache[1]["coarse"]))

    def test_corner_sample_dataset_cache_stores_input_sized_images(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            roi_dir = root / "roi" / "train"
            roi_dir.mkdir(parents=True)
        ...
        assert dataset._cache is not None
        self.assertEqual(tuple(dataset._cache[0]["image"].shape), (128, 128, 3))
