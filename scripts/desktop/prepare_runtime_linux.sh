...
TARGET_DIR="$ROOT_DIR/program/engine/vendor/linux/bin"
mkdir -p "$TARGET_DIR"
...
if [[ -n "${SCREEN_PDF_RUNTIME_SOURCE_DIR:-}" ]]; then
  SRC="$SCREEN_PDF_RUNTIME_SOURCE_DIR/linux/bin"
  ...
  echo "SCREEN_PDF_RUNTIME_SOURCE_DIR is required for Linux runtime preparation" >&2
  exit 1
fi

chmod +x "$TARGET_DIR/python3" "$TARGET_DIR/tesseract" "$TARGET_DIR/gs"
printf 'Prepared Linux runtime under %s\n' "$TARGET_DIR"
