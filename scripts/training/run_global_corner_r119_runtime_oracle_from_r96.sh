...
OUTPUT_ROOT="${OUTPUT_ROOT:-training/runs/global_corner_r119_202604_runtime_oracle_from_r96_v1}"
...
python program/engine/global_corner_train.py \
  ...
  --sample-weight-power 0.4 \
  --teacher-guidance-weight 0.2 \
  --teacher-blend-ratio 1.0 \
  --teacher-corner-error-max 0.03 \
  --teacher-sample-error-max 0.03 \
  --teacher-target-mode oracle \
  --teacher-candidate-sources teacher,r3,roi,opencv \
